...
class Renderer:
    ...
    def __init__(self, model: "CivModel"):
        ...
        self.ax_civfood = self.fig.add_subplot(
            gs[1, 2]
        ...
        (self._res_food,) = self.ax_res.plot(
            [], [], color="#44bb44", label="food", lw=1.2
        )
        (self._res_minerals,) = self.ax_res.plot(
            [], [], color="#aaaaaa", label="minerals", lw=1.2
        )
        (self._res_wood,) = self.ax_res.plot(
            [], [], color="#aa7733", label="wood", lw=1.2
        )
        ...
        for y, label in [(0.08, "mild"), (0.36, "severe"), (0.69, "catastrophic")]:
            self.ax_epi.text(
                ...
                color="#888",
                fontsize=6,
                transform=self.ax_epi.transAxes,
                va="bottom",
            )
    ...
    def update(self, model: "CivModel") -> None:
        ...
        _apply_overlay(
            self._sc_disease, [c for c in all_cities if c._disease_hit_ticks > 0]
        )
        _apply_overlay(
            self._sc_famine,
        ...
        for tick, beta, _deaths in model._epidemic_log[n_drawn:]:
            ...
            color = plt.colormaps["RdYlGn_r"](beta / 3.0)
