...
def test_get_relation_is_symmetric(mini_model):
    ...
    assert mini_model.get_relation(
        civs[1].civ_id, civs[0].civ_id
    ) == mini_model.get_relation(civs[0].civ_id, civs[1].civ_id)
...
def test_trade_increases_relations(mini_model):
    ...
    ), f"Trade should increase relations; before={rel_before}, after={rel_after}"
...
def test_attack_decreases_relations(mini_model):
    ...
    assert (
        rel_after < rel_before
    ), f"Attack should decrease relations; before={rel_before}, after={rel_after}"
...
def test_trade_blocked_when_relations_below_threshold(mini_model):
    ...
    sender._do_trade()
    assert (
        sender.food_stock == food_before
    ), "Trade should be blocked when relations are very negative"
...
def test_relation_decay_moves_positive_score_toward_zero(mini_model):
    ...
    assert (
        rel_after < rel_before
    ), f"Positive relation should decay toward 0; before={rel_before}, after={rel_after}"
...
def test_relation_decay_moves_negative_score_toward_zero(mini_model):
    ...
    assert (
        rel_after > rel_before
    ), f"Negative relation should decay toward 0; before={rel_before}, after={rel_after}"
