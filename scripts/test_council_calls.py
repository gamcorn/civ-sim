...
TRAITS = CulturalTraits(
    aggressiveness=0.6, trust=0.4, innovation=0.5, tribalism=0.5, risk_tolerance=0.6,
)
...
async def main():
    ...
    test_cases = [
        ("think tag", "We should do X.\n</think>\n{\"analysis\": \"ok\", \"recommendation\": \"gather\", \"weight_requests\": {\"gather\": 0.7}, \"disagreement_level\": 0.1}"),
        ("text before", "Here is my analysis: {\"analysis\": \"ok\", \"recommendation\": \"gather\", \"weight_requests\": {}, \"disagreement_level\": 0.0}"),
        ("schema confusion", "The schema shows { \"action_name\": 0.5 }. My answer: {\"analysis\": \"ok\", \"recommendation\": \"research\", \"weight_requests\": {\"research\": 0.8}, \"disagreement_level\": 0.0}"),
    ]
    ...
    for spec in MINISTER_SPECS:
        for i in range(n_samples):
            ...
            result = await call_sector_minister(
                spec, FAKE_SNAPSHOT, TRAITS, client, MODEL, TIMEOUT,
                guided_json=False,
            )
            ...
            if has_weights:
                ok_count += 1
            print(f"  [{status}] {spec['name']} s{i+1}: weights={result.get('weight_requests', {})}")
    ...
    fake_sector_outputs = [
        {"name": "Minister of War", "recommendation": "fortify", "weight_requests": {"fortify": 0.5}, "disagreement_level": 0.2},
        {"name": "Minister of Economy", "recommendation": "gather", "weight_requests": {"gather": 0.7}, "disagreement_level": 0.1},
        {"name": "Minister of Science", "recommendation": "research", "weight_requests": {"research": 0.6}, "disagreement_level": 0.2},
        {"name": "Minister of Expansion", "recommendation": "expand", "weight_requests": {"expand": 0.4}, "disagreement_level": 0.4},
    ]
    for i in range(n_samples):
        result = await call_budget_minister(
            FAKE_SNAPSHOT, fake_sector_outputs, TRAITS, client, MODEL, TIMEOUT, guided_json=True,
        )
        print(f"  Budget s{i+1}: veto={result.get('veto')}, approved={result.get('approved_weights', {})}")
    print()

    # ── Chief (full council, guided_json) ─────────────────────────────
    print(f"## Chief of Staff (guided_json=True, timeout={CHIEF_TIMEOUT}s)")
    fake_budget = {"veto": False, "veto_reason": None, "approved_weights": {"gather": 0.7, "fortify": 0.5}}
    for i in range(n_samples):
        result = await call_chief(
            FAKE_SNAPSHOT, fake_sector_outputs, fake_budget, TRAITS,
            client, MODEL, CHIEF_TIMEOUT,
            round_num=2, max_rounds=2, guided_json=True,
        )
        if result:
